...
class TCPProxy(TCPConnectionHandler):
    ...
    async def _keep_alive(self):
        while True:
            ...
            if self._is_connected and (self._origin is not None):
    ...
    async def _reconnect_origin(self):
        logging.info(f"Scheduled to reconnect { self._name } in { self._reconnect_interval } seconds ...")
    ...
    def on_data_received(
        ...
        logging.info(f"{ self._name } received { len(data) } bytes")
        try:
